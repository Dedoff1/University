...
begin
  ...
  write('Многочлены ');
  if not Equality(A,B) then write('не ');
  write('равны');
  writeln;
  writeln;

  write('Укажите значение для x: ');
  readln(x);
  writeln('Значение первого многочлена при x равном ', x, ': ',Meaning(A,x));
  writeln('Значение второго многочлена при x равном ', x, ': ',Meaning(B,x));
  writeln;
