...
public partial class MainForm : Form
{
    ...
    void OpenFilePlain_Click(object sender, EventArgs e)
    {
        if (OpenFileDialog.ShowDialog() != DialogResult.Cancel)
        {
            ...
            {
                // Считываем содержимое файла до определенного символа (например, символа ';')
                string text = reader.ReadToEnd(); // Считываем весь текст файла

                int index = text.IndexOf(';'); // Находим индекс первого вхождения символа ';'
                if (index >= 0)
                {
                    string textBeforeSymbol = text.Substring(0, index); // Получаем текст до символа ';'
                    PlainText.Text= textBeforeSymbol; // Выводим текст до символа ';'
                }
            }
