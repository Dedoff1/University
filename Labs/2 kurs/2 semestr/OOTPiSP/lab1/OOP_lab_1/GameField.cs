...
namespace OOP_lab_1
{
    public class GameField : Rectangle
    {
        ...
        public static GameField GenerateObjects(int width, int height, int minX, int minY, int maxX, int maxY)
        {
            ...
            int border = 10;
            GameField gameField = new GameField(minX + border/2+100, minY + border/2,minX + border/2, minY + border/2, new int[] {rand.Next(255), rand.Next(255), rand.Next(255)}, new int[] {rand.Next(255), rand.Next(255), rand.Next(255)}, border, width - border*2, height - border*2);
            ...
            for (int i = 0; i < 10; i++)
            {
                AddEquilateralTriangle(index, minX+100, minY+100, maxX-100, maxY-100, minB, maxB, gameField.arr, rand);
                index++;
            }
            for (int i = 0; i < 10; i++)
            {
                AddIsoscelesTriangle(index, minX+100, minY+100, maxX-100, maxY-100, minB, maxB, gameField.arr, rand);
                index++;
            }
            for (int i = 0; i < 10; i++)
            {
                AddRectangle(index, minX+100, minY+100, maxX-100, maxY-100, minB, maxB, gameField.arr, rand);
                index++;
            }
            for (int i = 0; i < 10; i++)
            {
                AddSection(index, minX+100, minY+100, maxX-100, maxY-100, minB, maxB, gameField.arr, rand);
                index++;
            }
            for (int i = 0; i < 10; i++)
            {
                AddSquare(index, minX+100, minY+100, maxX - 100, maxY - 100, minB, maxB, gameField.arr, rand);
                index++;
            }
            for (int i = 0; i < 10; i++)
            {
                AddTriangle(index, minX+100, minY+100, maxX-100, maxY-100, minB, maxB, gameField.arr, rand);
                index++;
            }
            return gameField;
        }
