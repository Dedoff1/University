...
namespace oop3.DisplayObjects
{
    internal class CircleObject : EllipseObject
    {
        ...
        public void CircleCollide(CircleObject obj) {
            ...
            var vx = ((temp1+temp2)/temp3)*Math.Cos(phiRad)+this.velModulo*Math.Sin(theta1Rad-phiRad)*Math.Cos(phiRad+Math.PI/2) + 75;
            var vy = ((temp1+temp2)/temp3)*Math.Sin(phiRad)+this.velModulo*Math.Sin(theta1Rad-phiRad)*Math.Sin(phiRad+Math.PI/2) + 75;
