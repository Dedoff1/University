...
namespace oop3.DisplayObjects
{
    public abstract class DisplayObject
    {
        ...
        // acceleration parametrs
        public int accModule;
        public int accAng;
        ...
        public void Move(double deltaTime)
        {
            ...
            double time = processingTime;

            double newX = startX + (time * (Math.Cos(angleAcc) * accModulo * time / 2 + Math.Cos(angleVel) * velModulo));
            double newY = startY + (time * (Math.Sin(angleAcc) * accModulo * time / 2 + Math.Sin(angleVel) * velModulo));




            ShiftObject(newX - anchorX, newY - anchorY);
        }
