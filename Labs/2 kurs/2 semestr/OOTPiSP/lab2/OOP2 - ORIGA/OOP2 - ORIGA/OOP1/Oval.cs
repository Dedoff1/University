...
namespace OOP1
{
    public class Oval : DisplayObject
    {
        ...
        public Oval(double x1, double y1, double x2, double y2, int fillColorR, int fillColorG, int fillColorB, int borderColorR, int borderColorG, int borderColorB, int borderWidth, PictureBox pictureBox, int vel, int angle, int accelX, int accelY)
        ...
        public override void Draw(Graphics g)
        {
            ...
            {
                g.FillEllipse(brush, (float) x1, (float)y1, width, height);
            }
            using (Pen pen = new Pen(borderColor, borderWidth))
            {
                g.DrawEllipse(pen, (float)x1, (float)y1, width, height);
            }
        }
