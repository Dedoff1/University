...
Begin
  Repeat
    ...
    Try
      ...
      while (M < 1) or (M > 1000000) do
        writeln('Количество элементов может быть в прделах от 1 до 1.000.000!');
      SetLength(Arr, M);
    ...
    Except
      ...
      writeln('Значение элементов последовательности могут быть только целым!');
      writeln('_______________________________________________');
      writeln;
  ...
  begin
    ...
    begin
      ...
      writeln(Arr[I],' - значение повторяющихся элементов.');
  ...
  If IsRepetition then
    writeln('Повторяющихся элементов не найдено.')


// Otherwise, an additional value of
  // the variable is output.
  Else
    writeln(Max, ' - значение максимального повторяющегося элемента.');
